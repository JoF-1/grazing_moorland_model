...
#NB THESE ARE FOR TESTING INORE IN RUNS
#Heather_grass <- ssimLibrary("C:/Local data/Modelling/Heather_grasssimplev3.ssim")
#Heather_grass_grazing <- rsyncrosim::project(Heather_grass, project = "Heather, wood, U4 grass, transition groups")
#Heather_scenario <- rsyncrosim::scenario(Heather_grass_grazing, scenario ="Test of transition groups")
...
#resultsscenario <- scenario(Heather_grass_grazing, scenario = 129)
#NPP_test <- datasheetSpatRaster(
 # resultsscenario,
  ##datasheet = 'stsim_OutputSpatialStateAttribute',
  #filterColumn = 'StateAttributeTypeId',
 # filterValue = 'Net Primary Production',
  #iteration=1,
  #timestep=1)
...
#(resultsscenario, name = 'stsim_OutputSpatialStateAttribute')
#datasheet(resultsscenario, name = 'stsim_OutputSpatialState')
#datasheet(resultsscenario, name = 'stsim_OutputStock')

#SpatialNPP <- 'stsim_OutputSpatialStateAttribute'
#SpatialNPPcolumn <- 'StateAttributeTypeId'
#SpatialNPPvlaue <- 'Net Primary Production'
...
TransitionGroupid_nograzing <- "No grazing group"
TransitionGroupid_vlightgrazing <- "Very light grazing group"
TransitionGroupid_lightgrazing <- "Light grazing group"
TransitionGroupid_lightmodgrazing <- "Light-moderate grazing group"
TransitionGroupid_moderategrazing <- "Moderate grazing group"
TransitionGroupid_modheavygrazing <- "Moderate-heavy grazing group"
TransitionGroupid_heavygrazing <- "Heavy grazing group"
TransitionGroupid_heavyvheavygrazing <- "Heavy to very heavy grazing group"
TransitionGroupid_vheavygrazing <- "Very heavy grazing group"
...
#load in NPP from previous timestep and results scenario
NPP <- datasheetSpatRaster(
  myScenario,
  datasheet = 'stsim_OutputSpatialStateAttribute',
  filterColumn = 'StateAttributeTypeId',
  filterValue = 'Net Primary Production',
...
#load in standing biomass from previous timestep and results scenario
biomass <- datasheetSpatRaster(
  myScenario,
  datasheet = 'stsim_OutputSpatialStateAttribute',
  filterColumn = 'StateAttributeTypeId',
  filterValue = 'Standing biomass',
  iteration=iteration,
  timestep=prevTimestep)
...
#defining monthly fractions of annual NPP for each habitat type, as set out in Armstrong et al and other research (see documentation)
...
#Reclassify habitats according to normalised grazing preference (according to distribution of preferred forage, grasses)
grazing_preference <- classify(inputRaster, rcl = matrix(c(
  3, 1,  # U4 grassland - most palatable
...
## Three live pools (0,1,2, 3 months) which sheep can graze before growth becomes “standing”.      
## Eat fresh grass first, then *forced* 5 % of demand must come from heather, then older live pools, finally capped standing pool. Standing pool capped at 50%
...
#Create empty storage for monthly utilisation maps and an offtake summary log
...
#grazing loop for month m in timestep T
for (m in graze_months) {
  ...
  ## snapshot of all live growth before grazing (for utilisation map calc later)
  ...
  ## Helper function to calculate how much sheep can eat from a biomass pool, constrained by supply, demand and physical cap.
  ...
  ## 3. grass from live0: sheep eat as much as possible from this month's live grass growth -----------------------------------------------------
  take0 <- eat_pool(live0) ; live0 <- live0 - take0
  
  ## 4. Sheep are forced to eat at least 5% of demand from heather; adjusts ddm_need and phys_cap accordingly --------------------------------------------
  forced_frac <- 0.05
  need_DDM_H  <- ddm_need * forced_frac
  need_DM_H   <- safe_div(need_DDM_H, digest_heather)
  
  takeH_forced <- rmin3(live0 * mask_heath, need_DM_H, phys_cap)
  ...
  ## 5.Sheep then eat from 1, 2 and 3-month-old live biomass pool -----------------------------------------------------
  take1 <- eat_pool(live1) ; live1 <- live1 - take1
  take2 <- eat_pool(live2) ; live2 <- live2 - take2
  take3 <- eat_pool(live3); live3 <- live3 - take3
  consumed_new <- consumed_new + take1 + take2 + take3  # final live tally
  
  ## 6. Sheep then eat from the standing biomass pool (e.g., older unpalatable material) (50 % cap) --------------------------------------------
  ...
  ## 7. Shift live pools: 2-month-old becomes standing biomass, and live pools age forward one step ---------------------------
  biomass <- biomass + live3
  live3 <- live2
  live2 <- live1
  live1 <- live0
  live0 <- live0 * 0
  ...
  take2_extra <- can_take * (live2 / total_surplus)
  take3_extra <- can_take * (live3 / total_surplus)
  live0 <- live0 - take0_extra
  live1 <- live1 - take1_extra
  live2 <- live2 - take2_extra
  live3 <- live3 - take3_extra
  consumed_new <- consumed_new + take0_extra + take1_extra + take2_extra + take3_extra
  
  
  ## 9. Calculate proportion of live growth consumed (utilisation)  ------------------------------------------
...
# Unmet demand remaining after all grazing attempts
unmet_ddm <- ddm_need
unmet_cells <- which(values(unmet_ddm) > 0)

surplus_pool <- live0 + live1 + live2
surplus_mask <- surplus_pool > 0
